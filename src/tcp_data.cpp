...
void tcp_consume_ofo_queue(Tcp_Sock *tsk) {
  ...
  while ((skb = tsk->ofo_queue.queue_first_entry()) &&
         tcb->rcv_nxt == skb->seq) {
...
int tcp_data_dequeue(Tcp_Sock *tsk, void *user_buff, const int &userlen) {
  ...
  while (!sk->receive_queue.isEmpty() && rlen < userlen) {
    ...
    int dlen = static_cast<int>(rlen + skb->data_len) > userlen
                   ? (userlen - rlen)
                   : skb->data_len;
    ...
    skb->payload += dlen;
    rlen += dlen;
    user_buff = static_cast<char *>(user_buff) + dlen;
...
int tcp_data_queue(Tcp_Sock *tsk, Tcp_Hdr * /* tcphdr */, SkBuff *skb) {
  ...
  if (skb->seq == tcb->rcv_nxt) {
    ...
    /*
        Per RFC5581, send an immediate duplicate ACK on receiving an
       out-of-order segment to notify the sender of the missing sequence number.
    */
    tcp_send_ack(sk);
  }
