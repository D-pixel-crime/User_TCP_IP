...
static Net_Family inet([](Socket *sock, int protocol) -> int {
...
static Sock_Ops inet_stream_ops(
    ...
    [](Socket *sock) -> int { return inet_close(sock); },
    [](Socket *sock) -> int { return inet_free(sock); },
    [](Socket *sock) -> int { return inet_abort(sock); }, NULL,
    [](Socket *sock, sockaddr *__restrict_arr addr,
       socklen_t *__restrict_arr addr_len) -> int {
...
int inet_socket(Socket *sock, const int &protocol) { return 0; }

int inet_stream_connect(Socket *sock, const sockaddr *addr, int addr_len,
                        int flags) {
  Sock *sk = sock->sk;
  ...
  case Socket_State::SS_UNCONNECTED:
...

    {
      ...
      while (sock->state == Socket_State::SS_CONNECTING &&
             sk->err == -EINPROGRESS) {
        socket_release(sock);
        sock->sleep.sleep();
...
int inet_getpeername(Socket *sock, sockaddr *__restrict_arr address,
  ...
  *address_len = sizeof(sockaddr_in);
  ...
  inet_dbg(sock,
           std::format(
               "getpeername sin_family-{} sin_port-{} sin_addr-{} addr_len-{}",
               res->sin_family, ntohs(res->sin_port),
               ntohl(res->sin_addr.s_addr), *address_len));

  return 0;
}

int inet_getsockname(Socket *sock, sockaddr *__restrict_arr address,
                     socklen_t *__restrict_arr address_len) {
  Sock *sk = sock->sk;
  if (!sk) {
    return -1;
