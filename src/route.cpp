...
Rt_Entry::Rt_Entry(uint32_t _dest, uint32_t _gateway, uint32_t _netmask,
                   uint32_t _metric, uint8_t _flags, Network_Device *_dev)
    : dev(_dev), dest(_dest), gateway(_gateway), netmask(_netmask),
      metric(_metric), flags(_flags) {};

IntrusiveQueue<Rt_Entry> rtEntry_queue(Rt_Entry::getOffset__list_node());

void init_routes() {
  Rt_Entry *loopbackRt_Entry =
      new Rt_Entry(loopback->addr, 0, 0xff000000, RT_LOOPBACK, 0, loopback);
  rtEntry_queue.queue_add_tail(&loopbackRt_Entry->list_node);

  Rt_Entry *netdevRt_Entry =
      new Rt_Entry(netdev->addr, 0, 0xffffff00, RT_HOST, 0, netdev);
  rtEntry_queue.queue_add_tail(&netdevRt_Entry->list_node);

  Rt_Entry *tapAddrRt_Entry =
      new Rt_Entry(0, parse_ipv4_string(TUNTAP_Interface::tap_addr), 0,
                   RT_GATEWAY, 0, netdev);
...
void free_routes() {
  rtEntry_queue.clear_queue([](Rt_Entry *entry) { delete entry; });
}