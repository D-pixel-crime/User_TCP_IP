...
static inline void list_for_each_safe(list_head *head, FUNC &&f) {
  for (list_head *pos = head->next, *p = pos->next; pos != head;
       pos = p, p = pos->next) {
    if (f(pos)) {
      break;
    }
...
template <typename T> class IntrusiveQueue {
private:
  list_head head;
  size_t len;
  size_t offset;
...
public:
  ...
  list_head *get_queue_list_head() { return &head; }
  ...
  template <typename CLEANER> void clear_queue(CLEANER &&cleaner) {
