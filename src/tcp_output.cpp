...
SkBuff *tcp_alloc_skb(const int &opt_len, const int &size) {
  int reserveSize = Eth_Hdr::getSize() + Ip_Hdr::getSize() +
                    Tcp_Hdr::getSize() + opt_len + size;
...
int tcp_send_next(Sock *sk, const int &amount) {
  ...
  list_for_each_safe(
      sk->write_queue.get_queue_list_head(), [&](list_head *pos) {
        if (++i < amount) {
          return true;
        }
...
int tcp_send_syn(Sock *sk) {
  if (sk->state != (int)Tcp_State::TCP_SYN_SENT &&
      ...
      sk->state != (int)Tcp_State::TCP_LISTEN) {
    print_err(
        "ERR(tcp_send_sync): Socket not in correct state (closed or listen)");
    return 1;
  }
...
void tcp_select_initial_window(uint32_t *rcv_wnd) { *rcv_wnd = 44477; }
...
void *tcp_connect_rto(void *arg) {
...

  {
    ...
    if (sk->state == (int)Tcp_State::TCP_SYN_SENT) {
      ...
      print_err("ERR(tcp_connect_rto): TCP connect RTO triggered even when not "
                "in SYNSENT.");
    }
...
void *tcp_retransmission_timeout(void *arg) {
  ...
  if (tsk->rto > 60000) {
    ...
    tsk->rto *= 2;
    tsk->backoff++;
    tsk->retransmit = Timer::create(
        tsk->rto, [tsk]() { tcp_retransmission_timeout(tsk); },
...
void tcp_rearm_rto_timer(Tcp_Sock *tsk) {
  ...
  if (sk->state == (int)Tcp_State::TCP_SYN_SENT) {
    tsk->retransmit = Timer::create(
        TCP_SYN_BACKOFF << tsk->backoff, [tsk]() { tcp_connect_rto(tsk); },
        Timer_Type::Trackable);
  } else {
    tsk->retransmit = Timer::create(
        tsk->rto, [tsk]() { tcp_retransmission_timeout(tsk); },
...
int tcp_send(Tcp_Sock *tsk, const void *buff, const int &len) {
  ...
  while (slen > 0) {
    ...
    buff = static_cast<const uint8_t *>(buff) + dlen;
