...
public:
  ...
  /*
     ...
     I think its better to use 'SkBuff*' using 'shared_ptr', then no need to
     keep this in check.
  */
  ...
  static size_t getOffset__list_node() { return offsetof(SkBuff, list_node); }
...
inline void free_skb(SkBuff *skb) {
  if (skb->refcnt <= 1) {
    delete skb;
    return;
  }
