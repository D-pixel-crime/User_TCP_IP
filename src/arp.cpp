...
IntrusiveQueue<Arp_Cache_Entry>
    arp_cache_entry_queue(Arp_Cache_Entry::getOffset__list_node());

SkBuff *arp_alloc_skb() {
  SkBuff *skb =
      new SkBuff(Eth_Hdr::getSize() + Arp_Hdr::getSize() + Arp_Ipv4::getSize());
  skb->reserve_headroom(Eth_Hdr::getSize() + Arp_Hdr::getSize() +
                        Arp_Ipv4::getSize());
...
int insert_arp_translation_table(Arp_Hdr *hdr, Arp_Ipv4 *data) {
  Arp_Cache_Entry *new_entry =
      new Arp_Cache_Entry(hdr->hwtype, data->sip, data->smac, ARP_RESOLVED);

  {
    std::unique_lock<std::mutex> lock(arp_cache_mutex);
...
int update_arp_translation_table(Arp_Hdr *hdr, Arp_Ipv4 *data) {
  ...
  int found = 0;
  list_for_each(arp_cache_entry_queue.get_queue_list_head(),
                [&](list_head *pos) {
                  Arp_Cache_Entry *entry = list_entry<Arp_Cache_Entry>(
                      pos, Arp_Cache_Entry::getOffset__list_node());
...
uint8_t *arp_get_hwaddr(const uint32_t &sip) {
  ...
  list_for_each(
      arp_cache_entry_queue.get_queue_list_head(), [&](list_head *pos) {
        Arp_Cache_Entry *arp_entry = list_entry<Arp_Cache_Entry>(
            pos, Arp_Cache_Entry::getOffset__list_node());
...
void free_arp() {
  arp_cache_entry_queue.clear_queue(
      [](Arp_Cache_Entry *entry) { delete entry; });
}
