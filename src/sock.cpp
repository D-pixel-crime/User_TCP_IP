...
Net_Ops::Net_Ops(
    std::function<Sock *(int protocol)> _allock_sock,
    ...
    std::function<int(Sock *sk)> _close, std::function<int(Sock *sk)> _abort)
    : alloc_sock{_allock_sock}, init{_init}, connect{_connect},
      disconnect{_disconnect}, write{_write}, read{_read},
      recv_notify{_recv_notify}, close{_close}, abort{_abort} {}
