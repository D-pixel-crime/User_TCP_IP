...
int tcp_verify_segment(Tcp_Sock *tsk, Tcp_Hdr *tcphdr, SkBuff *skb) {
  ...
  if (tcphdr->seq < tcb->rcv_nxt ||
      tcphdr->seq > (tcb->rcv_nxt + tcb->rcv_wnd)) {
...
inline int tcp_discard(Tcp_Sock * /* tsk */, SkBuff *skb,
                       Tcp_Hdr * /* tcphdr */) {
  free_skb(skb);
  return 0;
}

inline int tcp_listen(Tcp_Sock * /* tsk */, SkBuff *skb,
                      Tcp_Hdr * /* tcphdr */) {
  free_skb(skb);
  return 0;
}
  ...
  case Tcp_State::TCP_FIN_WAIT_2:
    if (expected) {
      ...
      if (!tsk->inflight && pending > 0) {
        ...
        tsk->delack = Timer::create(
            200, [tsk]() { tcp_send_delack(&tsk->sk); }, Timer_Type::Trackable);
      }
    }
    break;
  default:
...
int tcp_receive(Tcp_Sock *tsk, void *buff, const int &len) {
  ...
  while (rlen < len) {
    currlen =
        tcp_data_dequeue(tsk, static_cast<uint8_t *>(buff) + rlen, len - rlen);
    rlen += currlen;
