...
inline constexpr std::array<const std::string_view, 11> tcp_dbg_states = {
    ...
    "TCP_FIN_WAIT_1", "TCP_FIN_WAIT_2", "TCP_CLOSE",        "TCP_CLOSE_WAIT",
...
public:
  ...
  static size_t getSize() { return sizeof(Tcp_Hdr); }

  static size_t getDoffset() { return getSize() / 4; }

  auto tcp_hlen() { return hl << 2; }
};

template <typename T> Tcp_Hdr *tcp_hdr(T *skb) {
  return reinterpret_cast<Tcp_Hdr *>(skb->head + Eth_Hdr::getSize() +
                                     Ip_Hdr::getSize());
}
 ...
 * @note Explicitly backed by uint8_t for safe serialization and memory
 * efficiency.
 */
enum class Tcp_State : uint8_t {
  ...
  /** @brief Active close complete; waiting 2*MSL to ensure remote peer received
     ACK. */
  TCP_TIME_WAIT
};
...
template <typename T> Tcp_Sock *tcp_sk(T *sk) {
  return reinterpret_cast<Tcp_Sock *>(sk);
}

inline void tcp_in_dbg(Tcp_Hdr *tcphdr, Sock *sk, SkBuff *skb) {
  print_debug(std::format(
      "TCP {}.{}.{}.{}.{} > {}.{}.{}.{}.{}: Flags [S{}A{}P{}F{}R{}], seq "
      "{}:{}, ack {}, win {} rto {} boff {}",
      static_cast<uint8_t>(sk->daddr >> 24),
      static_cast<uint8_t>(sk->daddr >> 16),
      static_cast<uint8_t>(sk->daddr >> 8),
      static_cast<uint8_t>(sk->daddr >> 0), sk->dport,
      static_cast<uint8_t>(sk->saddr >> 24),
      static_cast<uint8_t>(sk->saddr >> 16),
      static_cast<uint8_t>(sk->saddr >> 8),
      static_cast<uint8_t>(sk->saddr >> 0), sk->sport,
      ...
      static_cast<uint8_t>(tcphdr->rst), tcphdr->seq - tcp_sk(sk)->tcb.irs,
      tcphdr->seq + skb->data_len - tcp_sk(sk)->tcb.irs,
      tcphdr->ack_seq - tcp_sk(sk)->tcb.iss, tcphdr->win, tcp_sk(sk)->rto,
      tcp_sk(sk)->backoff));
}
...
inline void tcp_out_dbg(Tcp_Hdr *tcphdr, Sock *sk, SkBuff *skb) {
  print_debug(std::format(
      ...
      static_cast<uint8_t>(tcphdr->syn), static_cast<uint8_t>(tcphdr->ack),
      static_cast<uint8_t>(tcphdr->psh), static_cast<uint8_t>(tcphdr->fin),
      static_cast<uint8_t>(tcphdr->rst), tcphdr->seq - tcp_sk(sk)->tcb.iss,
      tcphdr->seq + skb->data_len - tcp_sk(sk)->tcb.iss,
      tcphdr->ack_seq - tcp_sk(sk)->tcb.irs, tcphdr->win, tcp_sk(sk)->rto,
      tcp_sk(sk)->backoff));
}
...
inline void tcpsock_dbg(std::string_view msg, Sock *sk) {
  print_debug(std::format(
      ...
      "rto {} boff {}) state {}: {}",
      sk->sport, static_cast<uint8_t>(sk->daddr >> 24),
      static_cast<uint8_t>(sk->daddr >> 16),
      static_cast<uint8_t>(sk->daddr >> 8),
      static_cast<uint8_t>(sk->daddr >> 0), sk->dport,
      ...
      sk->receive_queue.size(), sk->write_queue.size(), tcp_sk(sk)->rto,
      tcp_sk(sk)->backoff, tcp_dbg_states[sk->state], msg));
}
...
int tcp_v4_connect(Sock *sk, const sockaddr *addr, const int &addr_len,
                   const int &flags);
...
int tcp_udp_checksum(const uint32_t &saddr, const uint32_t &daddr,
                     const uint8_t proto, const uint8_t *data,
                     const uint16_t &len);
...
int tcp_write_syn_options(Tcp_Hdr *tcphdr, Tcp_Options *opts,
                          const int &opt_len);
