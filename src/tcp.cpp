...
Tcp_Sock::Tcp_Sock()
    : rmss{1460}, smss{536}, sackok{1},
      ofo_queue(IntrusiveQueue<SkBuff>(SkBuff::getOffset__list_node())) {
  sk.state = (int)Tcp_State::TCP_CLOSE;
}

Sock *tcp_alloc_sock() { return reinterpret_cast<Sock *>(new Tcp_Sock()); }
...
int tcp_init_sock(Sock * /*sk*/) { return 0; }

void tcp_set_state(Sock *sk, const uint32_t &state) { sk->state = state; }
...
uint16_t generate_port() {
  ...
  if (::bind(temp_fd, reinterpret_cast<struct sockaddr *>(&addr),
             sizeof(addr)) < 0) {
    ::close(temp_fd);
  ...
  if (::getsockname(temp_fd, reinterpret_cast<struct sockaddr *>(&addr), &len) <
      0) {
    ::close(temp_fd);
...
int tcp_disconnect(Sock * /*sk*/, const int & /*flags*/) { return 0; }
...
int tcp_read(Sock *sk, const void * /*buff*/, const int & /*len*/) {
  ...
  case Tcp_State::TCP_SYN_RECEIVED:
    /*
        Queue for requests after entering ESTABLISHED state. If full, return
  ...
  case Tcp_State::TCP_CLOSE_WAIT:
    /*
        If no text is awaiting delivery, return "error: connection closing".
       Otherwise, satisfy the RECEIVE with any remaining text.
    */
...
int tcp_recv_notify(Sock *sk) { return sk->recv_wait.wakeup(); }
...
void tcp_enter_time_wait(Sock *sk) {
  ...
  tcp_clear_timers(sk);
  tsk->linger = Timer::create(
      TCP_2MSL, [sk]() { tcp_linger(sk); }, Timer_Type::Trackable);
}
...
void tcp_rearm_user_timeout(Sock *sk) {
  ...
  tsk->linger->cancel();
  tsk->linger = Timer::create(
      TCP_USER_TIMEOUT, [sk]() { tcp_user_timeout(sk); },
...
static Net_Ops
tcp_ops([](int /*protocol*/) -> Sock * { return tcp_alloc_sock(); },
        [](Sock *sk) -> int { return tcp_v4_init_sock(sk); },
        [](Sock *sk, const sockaddr *addr, int addr_len, int flags) -> int {
          return tcp_v4_connect(sk, addr, addr_len, flags);
        },
        [](Sock *sk, int flags) -> int { return tcp_disconnect(sk, flags); },
        ...
        [](Sock *sk) -> int { return tcp_recv_notify(sk); },
        [](Sock *sk) -> int { return tcp_close(sk); },
        [](Sock *sk) -> int { return tcp_abort(sk); });
...
void tcp_clear_queues(Tcp_Sock *tsk) { skb_queue_free(&tsk->ofo_queue); }
...
void tcp_in(SkBuff *skb) {
  ...
  if (!sk) {
    print_err("ERR(tcp_in): No TCP-Socket for sport-{} and dport-{}.",
