...
/**
 * Binds @param dev to @param addr, so that the network-interface can be used for routing
 */
static int set_interface_address(const std::string &dev, const std::string &addr)
{
 ...
 */
static int set_interface_route(const std::string &dev, const std::string &route)
{
...
{
    tuntap_interface = new TUNTAP_Interface();
}
